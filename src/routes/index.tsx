...
import { z } from "zod";
import { ArrowDown, Building2, Check, ChevronRight, Construction, ExternalLink, LockKeyhole, Mail, MapPin, MessageCircle, Phone, ShieldCheck, Sparkles, Users, Waves, Wrench } from "lucide-react";
...
function HomePage() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    ].map(([Icon, title, text]) => { const C = Icon as typeof LockKeyhole; return <div key={String(title)} className="flex min-w-0 gap-4 sm:gap-5"><C className="mt-1 h-6 w-6 shrink-0 text-gold" /><div className="min-w-0"><h3 className="text-lg text-cream lg:text-xl">{String(title)}</h3><p className="mt-2 text-sm leading-7 text-muted-foreground sm:text-base">{String(text)}</p></div></div>; })}</div></div><div className="reveal flex min-w-0 flex-col"><iframe className="min-h-[360px] w-full flex-1 border-0 grayscale-[20%] sm:min-h-[420px]" loading="lazy" title="Ubicación exacta de Home Wonder Safe" src="https://www.google.com/maps?q=Home+Wonder+Safe,-12.0632762,-76.9342491&z=17&output=embed" allowFullScreen referrerPolicy="no-referrer-when-downgrade" /><Button asChild variant="goldOutline" className="mt-4 self-start"><a href={MAPS_URL} target="_blank" rel="noreferrer">Abrir en Maps <ExternalLink /></a></Button></div></div></div></section>

    <section className="px-5 py-20 sm:px-8 md:px-10 md:py-28"><div className="mx-auto w-full max-w-7xl min-w-0"><SectionHeading number="04" eyebrow="Distribución & tipologías" title="Privacidad absoluta" subtitle="Solo 1 departamento por piso. Cinco familias en todo el edificio." /><div className="grid min-w-0 items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,34rem)]"><div className="grid min-w-0 gap-5">{[
    ...
    ].map((item) => <article key={item[0]} className="reveal min-w-0 border border-gold/30 bg-night-soft p-6 sm:p-7 md:p-8"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4"><h3 className="min-w-0 text-2xl text-cream lg:text-3xl">{item[0]}</h3><span className="numeric shrink-0 font-display text-2xl text-gold lg:text-3xl">{item[1]}</span></div><div className="my-6 h-px bg-gold/20" /><p className="flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[2]}</p><p className="mt-3 flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[3]}</p></article>)}</div><figure className="reveal mx-auto grid w-full max-w-[34rem] min-w-0 grid-cols-2 gap-3"><img src={bedroom.url} loading="lazy" alt="Dormitorio principal iluminado de Home Wonder Safe" className="col-span-2 aspect-[16/10] w-full object-cover transition duration-700 hover:scale-[1.01]" /><img src={bedroomTwo.url} loading="lazy" alt="Dormitorio doble de Home Wonder Safe" className="aspect-square w-full object-cover" /><img src={bedroomThree.url} loading="lazy" alt="Dormitorio individual de Home Wonder Safe" className="aspect-square w-full object-cover" /><figcaption className="col-span-2 text-xs uppercase tracking-[0.15em] text-muted-foreground">Dormitorios amplios e iluminados</figcaption></figure></div></div></section>
    ...
    <section className="bg-night-soft px-5 py-20 sm:px-8 md:px-10 md:py-28"><div className="mx-auto w-full max-w-7xl min-w-0"><SectionHeading number="07" eyebrow="Equipamiento" title="Comodidad y estilo de vida superior" /><div className="grid min-w-0 gap-8">{[
