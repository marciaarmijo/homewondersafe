...
function SectionHeading({ number, eyebrow, title, subtitle, titleClassName = "" }: { number: string; eyebrow: string; title: string; subtitle?: string; titleClassName?: string }) {
...
function HeroVideo() {
  ...
  return <div className={`video-cover ${isPlaying ? "is-playing" : ""}`} aria-hidden="true"><iframe ref={iframeRef} src="https://www.youtube.com/embed/ti-4VKrdXg0?enablejsapi=1&autoplay=1&mute=1&playsinline=1&loop=1&playlist=ti-4VKrdXg0&controls=0&showinfo=0&rel=0&modestbranding=1&disablekb=1&fs=0&iv_load_policy=3" title="Home Wonder Safe" allow="autoplay; encrypted-media" tabIndex={-1} /><div className="video-shield" /></div>;
}
...
function HomePage() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    ].map((item) => <article key={item[0]} className="reveal min-w-0 border border-gold/30 bg-night-soft p-6 sm:p-7 md:p-8"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4"><h3 className="min-w-0 text-2xl text-cream lg:text-3xl">{item[0]}</h3><span className="numeric shrink-0 font-display text-2xl text-gold lg:text-3xl">{item[1]}</span></div><div className="my-6 h-px bg-gold/20" /><p className="flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[2]}</p><p className="mt-3 flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[3]}</p></article>)}</div><figure className="reveal mx-auto grid w-full max-w-[29rem] min-w-0 grid-cols-2 gap-3"><img src={bedroom.url} loading="lazy" alt="Dormitorio principal iluminado de Home Wonder Safe" className="col-span-2 h-48 w-full object-cover transition duration-700 hover:scale-[1.01] sm:h-56 lg:h-52" /><img src={bedroomTwo.url} loading="lazy" alt="Dormitorio doble de Home Wonder Safe" className="h-32 w-full object-cover sm:h-40 lg:h-36" /><img src={bedroomThree.url} loading="lazy" alt="Dormitorio individual de Home Wonder Safe" className="h-32 w-full object-cover sm:h-40 lg:h-36" /><figcaption className="col-span-2 text-xs uppercase tracking-[0.15em] text-muted-foreground">Dormitorios amplios e iluminados</figcaption></figure></div></div></section>
