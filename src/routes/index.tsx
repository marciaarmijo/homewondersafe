...
function HomePage() {
  ...
  return <main className="overflow-hidden bg-background text-foreground">
    ...
    ].map((item) => <article key={item[0]} className="reveal min-w-0 border border-gold/30 bg-night-soft p-6 sm:p-7 md:p-8"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4"><h3 className="min-w-0 text-2xl text-cream lg:text-3xl">{item[0]}</h3><span className="numeric shrink-0 font-display text-2xl text-gold lg:text-3xl">{item[1]}</span></div><div className="my-6 h-px bg-gold/20" /><p className="flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[2]}</p><p className="mt-3 flex gap-3 text-sm leading-7 text-muted-foreground sm:text-base"><Check className="h-5 w-5 shrink-0 text-gold" />{item[3]}</p></article>)}</div><figure className="reveal mx-auto grid w-full max-w-[29rem] min-w-0 grid-cols-3 gap-3 lg:h-[420px] lg:max-w-none lg:grid-rows-[minmax(0,2fr)_minmax(0,1fr)_auto]"><img src={bedroom.url} loading="lazy" alt="Dormitorio principal iluminado de Home Wonder Safe" className="col-span-3 h-48 w-full object-cover transition duration-700 hover:scale-[1.01] sm:h-56 lg:h-full lg:min-h-0" /><img src={bedroomTwo.url} loading="lazy" alt="Dormitorio doble de Home Wonder Safe" className="h-32 w-full object-cover sm:h-40 lg:h-full lg:min-h-0" /><img src={bedroomThree.url} loading="lazy" alt="Dormitorio individual de Home Wonder Safe" className="h-32 w-full object-cover sm:h-40 lg:h-full lg:min-h-0" /><img src="/habitacion1_1.png" loading="lazy" alt="Otro ángulo del dormitorio principal de Home Wonder Safe" className="h-32 w-full object-cover sm:h-40 lg:h-full lg:min-h-0" /><figcaption className="col-span-3 text-xs uppercase tracking-[0.15em] text-muted-foreground">Dormitorios amplios e iluminados</figcaption></figure></div></div></section>
